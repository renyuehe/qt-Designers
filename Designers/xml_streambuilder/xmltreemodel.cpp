    ...
    while (!m_streamReader.atEnd()) {
        ...
        switch (tt) {
            case QXmlStreamReader::StartElement: {
                ...
                item->setData(m_streamReader.lineNumber(), LineStartRole);  /* Custom data() */
                ...
                if (m_currentItem == 0) setItem(0, 0, item);                /* Set root item in model. */
                else m_currentItem->appendRow(item);                        /* Add as a child to the current item. */
                m_currentItem = item;
                break; }
